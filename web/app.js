...
async function init() {
  ...
  map.addControl(new maplibregl.AttributionControl({ compact: true, customAttribution: 'Timetables: Beograd Plus GTFS via data.gov.rs' }));
  ...
  // Beograd Plus numbers all three modes from one pool (trams 2–14,
  // trolleybuses 19–41, buses the rest), so a key IS the printed number and
  // nothing is stripped for display — see the same identity in build.mjs.
  const dispLine = (l) => l;
  ...
  function applyFilters() {
    ...
    if ((B || M) && !state.tram) {
      ...
      numField = state.tram && !(B || M) ? tramOnlyNumbersN : numberFieldN;
    }
  ...
  async function exportBBox(bbox, btn, doneLabel, opts) {
    ...
    try {
      ...
      const txt = '© OpenStreetMap contributors · OpenFreeMap · GTFS: Beograd Plus via data.gov.rs';
      ...
      a.download = `belgrade-transit_${d.getFullYear()}-${pad(d.getMonth() + 1)}-${pad(d.getDate())}_${pad(d.getHours())}${pad(d.getMinutes())}_${out.width}x${out.height}.png`;
  ...
  btnAtlas.addEventListener('click', async () => {
    ...
    try {
      ...
      const a = document.createElement('a');
      a.download = `belgrade-transit-giant_${d.getFullYear()}-${p2(d.getMonth() + 1)}-${p2(d.getDate())}_${Wf}x${Hf}.png`;
      a.href = URL.createObjectURL(blob);
  ...
  // NOT fitBounds(meta.bbox): the suburban lines reach Lazarevac and
  // Mladenovac, 50 km out, so fitting the data bbox would open on countryside
  // with the city as a small core. Open on central Belgrade instead.
  map.jumpTo({ center: [20.4570, 44.8125], zoom: 11.2 });
}
