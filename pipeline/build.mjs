...
// Belgrade: ONE Beograd Plus feed (data.gov.rs) split by route_type into two
// modes — buses+trolleybuses (3, 11) on the road graph and trams (0) on the
// tram graph. Trolleybuses ride the bus network in green per-line colors.
// There is no metro here, so the engine's metro treatment stays unused.
// Unlike Sofia, the operator numbers its modes from ONE pool — trams are
// 2–14, trolleybuses 19–41, buses everything else, with no number used twice
// — so the line keys are bare numbers and need no ТБ/ТМ-style prefixes.
...
import { buildLatinDict, latinLine } from './lib/serbian.mjs';

const ROOT = join(dirname(fileURLToPath(import.meta.url)), '..');

// Belgrade's OSM ways carry `name` in Cyrillic and `name:sr-Latn` beside it,
// so the Latin second line of a street label is read from the extract rather
// than transliterated — see lib/serbian.mjs. (No case dictionary is needed
// either: unlike CGM, Beograd Plus publishes its stop names properly cased.)
const latinDict = (() => {
  const docs = [];
  for (const f of ['data/osm/belgrade.json', 'data/osm/belgrade-rail.json']) {
    const p = join(ROOT, f);
    if (!existsSync(p)) continue;
    try { docs.push(JSON.parse(readFileSync(p, 'utf8'))); } catch { /* unreadable extract: skip */ }
  }
  return buildLatinDict(docs);
})();
...
// Beograd Plus numbers all three modes from ONE pool — trams hold 2–14,
// trolleybuses 19–41, buses everything else, no number used twice — so a key
// IS the number the city writes on the vehicle and nothing has to be stripped
// for display. (Sofia needs ТБ/ТМ prefixes here; Belgrade does not, and the
...
// ONE feed, TWO cfgs: routeTypes splits the Beograd Plus bundle per mode
// (3 = bus and 11 = trolleybus, both riding the road network, trolleybuses in
// green; 0 = tram on the tram graph). Without the filter `--all` on the bus
// mode would swallow the tram lines too. Belgrade has no metro, so the third
// cfg its siblings carry is absent here.
const MODES = [{
  mode: 'bus', label: 'buses', osmFile: 'data/osm/belgrade.json',
  graphMode: 'road', color: '#0059a9', colorDark: '#00294f',
  all: busAll, lines: busList.length ? busList : (busAll ? [] : ['26']),
  feeds: [
    {
      tag: 'bgp', dir: 'data/gtfs', routeTypes: ['3', '11'],
      mapKey: (sn) => norm(sn),
    },
  ],
}];
...
if (tramAll || tramSel.length) MODES.push({
  mode: 'tram', label: 'trams', osmFile: 'data/osm/belgrade-rail.json',
  ...
  feeds: [
    { tag: 'bgp', dir: 'data/gtfs', mapKey: (sn) => norm(sn), routeTypes: ['0'] },
  ],
});
...
async function processMode(cfg) {
  ...
  for (const feed of cfg.feeds) {
    ...
    for (const r of await readCsv(join(fdir, 'routes.txt'))) {
      ...
      // The feed's own route_color is deliberately ignored: Beograd Plus
      // colors by SERVICE CLASS (city blue, night indigo, express green,
      // suburban orange), which would collide head-on with this family's
      // colors-by-MODE convention — navy bus, green trolleybus, red tram
      // across all fourteen maps. The class shows in the line number instead
      // (N = night, E = express).
    ...
    for (const s of await readCsv(join(fdir, 'stops.txt'))) {
      ...
      // Beograd Plus publishes its stop names properly cased and in Serbian
      // Latin already ("Tadeuša Košćuška"), diacritics intact — the only
      // all-caps name in the feed is the genuine acronym IKEA, so titleCase
      // stays off and no case dictionary runs here.
...
const nameFeatures = mergeRuns(streetFeatures
  ...
  .map((r) => {
    // Second line of the label: the Latin reading Serbia signs its own streets
    // with — OSM's `name:sr-Latn` where it exists, transliteration where it
    // does not (see lib/serbian.mjs). A name that is already Latin adds
    // nothing and stays single-line.
    const latin = latinLine(r.name, latinDict);
    return {
      type: 'Feature',
      geometry: { type: 'LineString', coordinates: r.coords },
